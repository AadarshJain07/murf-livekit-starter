"""Outbound (SIP telephony) support for PrepPilot AI."""
...
logger = logging.getLogger("preppilot.outbound")


# --- call outcome logging -------------------------------------------------

def log_event(event: str, **fields: object) -> None:
    """Readable, greppable call-outcome logs."""
    extra = " ".join(
...
@dataclass
class OutboundJob:
    """Parsed outbound dial request coming from job metadata."""

    sip_call_to: str
    trunk_id: str
    participant_name: str = "PrepPilot AI"
...
def parse_outbound_job(metadata: str | None) -> OutboundJob | None:
    ...
    return OutboundJob(
        ...
        participant_name=str(
            data.get("participant_name") or "PrepPilot AI"
        ),
        user_id=str(
            data.get("user_id") or "demo-student-001"
        ),
        subject=str(
            data.get("subject") or "Physics"
        ),
        level=str(
            data.get("level") or "Class 11"
        ),
    )


class OutboundCallError(RuntimeError):
    """Raised when the call could not be connected."""
...
async def dial(
    ...
    try:
        ...
        log_event(
            ...
            to=job.sip_call_to,
            sip_status=status,
            sip_status_text=(e.metadata or {}).get(
                "sip_status",
                "",
            ),
            error=e.message,
        ...
        raise OutboundCallError(
            f"SIP connection failure: {e}"
        ) from e

    # IMPORTANT:
    # Do not use participant.identity here.
    # The returned SIPParticipantInfo does not expose that attribute
    # in the LiveKit version being used.
    log_event(
        "call connected",
        to=job.sip_call_to,
        participant=job.participant_name,
    )
